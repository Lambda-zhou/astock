...
def build_ai_context():
    ...
    if st.session_state.get('ai_include_lhb') and st.session_state.get('lhb_data') is not None:
        lhb_data = st.session_state['lhb_data']
        lhb_target_code = st.session_state.get('lhb_target_code') or '未知'
        lhb_query_type = st.session_state.get('lhb_query_type') or '未知'
        lhb_update_time = st.session_state.get('lhb_data_time')
        lhb_update_time_text = (
            lhb_update_time.strftime('%Y-%m-%d %H:%M:%S')
            if hasattr(lhb_update_time, 'strftime')
            else (str(lhb_update_time) if lhb_update_time else '未知')
        )
        if isinstance(lhb_data, pd.DataFrame):
            context_parts.append(
                "当前龙虎榜数据预览\n"
                f"股票代码: {lhb_target_code}\n"
                f"查询类型: {lhb_query_type}\n"
                f"更新时间: {lhb_update_time_text}\n"
                f"{lhb_data.head(20).to_csv(index=False)}"
            )
        else:
            context_parts.append(
                "当前龙虎榜数据预览\n"
                ...
                with st.spinner("正在查询龙虎榜数据..."):
                    try:
                        ...
                        if result is not None and (isinstance(result, pd.DataFrame) and not result.empty or 
                            ...
                            st.session_state.lhb_data = result
                            st.session_state.lhb_target_code = target_code
                            st.session_state.lhb_query_type = "search_in_lh"
                            st.session_state.lhb_data_time = pd.Timestamp.now()
                            st.success("该股票在龙虎榜中!")
                ...
                with st.spinner("正在获取详细数据..."):
                    try:
                        ...
                        if result is not None and (isinstance(result, pd.DataFrame) and not result.empty or 
                            ...
                            st.session_state.lhb_data = result
                            st.session_state.lhb_target_code = target_code
                            st.session_state.lhb_query_type = "find_lhb"
                            st.session_state.lhb_data_time = pd.Timestamp.now()
...
def handle_ai_chat():
    ...
    with st.expander("上下文设置", expanded=False):
        ...
        st.checkbox("把当前龙虎榜数据作为上下文", key="ai_include_lhb")
